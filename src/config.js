...
export const RENDER_P = {
  ...
  // PALETA DEL FONDO (hex, editable a mano). Render puro (no toca la sim). El fondo interpola abismo→pasto según la riqueza local de
  // vegetación; el refugio se superpone con su color. Tonos abisales y ligeramente ACROMÁTICOS (desaturados) a propósito.
  abyssColor: '#0a0e17',   // NO UI — agua profunda / pasto ARRASADO (extremo oscuro, casi negro azulado)
  pastoColor: '#103054',   // NO UI — PASTO rico (azul abisal desaturado) — extremo iluminado del campo de comida
  refugioColor: '#45260d', // NO UI — REFUGIO = verde alga apagado (gris-verde abisal)
...
export const GENOME_P = {
  ...
  // r/K: genes de historia de vida —umbral de cría (reproK) e inversión por cría (investFrac)— evolucionan bajo selección.
...
export const SIM_P = {
  ...
  grazeRefuge: 0.06,   // NO UI — reserva de rebrote: fracción de K de cada celda intocable por el pastoreo (no se pasta veg < grazeRefuge·K) → anti-sobrepastoreo.
  ...
  senesce: 0.00005,    // UI: Senescencia/vejez — coste metabólico ∝ edad; DEFAULT 5e-5 → vida finita + recambio; ≥2e-4 colapsa el cazador.
  fatWeight: 0.15,     // UI: Lastre de reservas — la energía almacenada penaliza vmax (vmax/(1+fatWeight·E/masa)); DEFAULT 0.15; su valor real es ecológico, no capar el atesoramiento.
  // --- resto (NO UI) ---
  ...
  massCostExp: 1.2,    // NO UI — exponente del coste de masa (super-lineal) → frena el BLOAT de cuerpos (1 = lineal/antiguo).
  moveCost: 0.004,     // NO UI — coste de nado ∝ drag·v² (energía → calor)
  mouthCost: 0.001,    // NO UI — coste de mantenimiento de boca ∝ mouthCap (→ calor) → boca bajo selección (sin él se infla ~50×, casi neutra).
  cooldown: 50,        // NO UI — enfriamiento reproductivo (ticks)
  eDensity: 0,         // NO UI — energía-en-biomasa; 0 = separación limpia materia/energía (eD>0 encarece la masa al nacer → poda el músculo → sésil).
  ...
  fleeSpeed: 1.0,      // UI: Escape por velocidad — la presa escapa si corre más que el depredador (×este factor) → la velocidad es defensa+ataque → mantiene el músculo bajo selección.
  coverStrength: 0.25, // UI: Cobertura del refugio — prob_escape = coverStrength·cover_local; regla FÍSICA (no un gen "esconderse"), la conducta emerge vía el sensor ∇cover.
  ...
  mateCompat: 0.5,     // NO UI — aislamiento PRE-cigótico: umbral = distancia FENOTÍPICA (masa/boca/presa-manejable) normalizada → estructura CLINAL (no especies discretas).
};
